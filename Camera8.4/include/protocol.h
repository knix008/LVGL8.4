...
public:
    ...
    SettingsMessage(float max_ratio, float max_deg, uint32_t min_size, float d_th, float f_th)
        : Message(MessageType::REQ_SET_SETTINGS),
          max_face_aspect_ratio(max_ratio),
          max_face_degree(max_deg),
          min_face_size(min_size),
          det_th(d_th),
          fas_th(f_th) {
        write_float(max_face_aspect_ratio);
        write_float(max_face_degree);
        write_uint32(min_face_size);
        write_float(det_th);
        write_float(fas_th);
        finalize();
    }
    ...
    static SettingsMessage from_message(const Message& msg) {
        size_t offset = 0;
        float max_ratio = msg.read_float(offset);
        float max_deg = msg.read_float(offset);
        uint32_t min_size = msg.read_uint32(offset);
        float d_th = msg.read_float(offset);
        float f_th = msg.read_float(offset);
        return SettingsMessage(max_ratio, max_deg, min_size, d_th, f_th);
    }
};

/**
 * @brief Detect faces control request
 */
class DetectFacesMessage : public Message {
public:
    bool enable;

    DetectFacesMessage(bool enable_detection)
        : Message(MessageType::REQ_DETECT_FACES),
          enable(enable_detection) {
        write_bool(enable);
        finalize();
    }

    static DetectFacesMessage from_message(const Message& msg) {
        size_t offset = 0;
        bool enable = msg.read_bool(offset);
        return DetectFacesMessage(enable);
    }
};

/**
 * @brief Face Anti-Spoofing control request
 */
class FASControlMessage : public Message {
public:
    bool enable;

    FASControlMessage(bool enable_fas)
        : Message(enable_fas ? MessageType::REQ_FAS_ON : MessageType::REQ_FAS_OFF),
          enable(enable_fas) {
        finalize();
    }

    static FASControlMessage from_message(const Message& msg) {
        return FASControlMessage(msg.header.get_type() == MessageType::REQ_FAS_ON);
    }
};
...
public:
    ...
    StatusResponse(bool cam_running, bool rec_enabled, bool training,
                  ...
                  float max_ratio = 0.0f, float max_deg = 0.0f, uint32_t min_size = 0,
                  float d_th = 0.0f, float f_th = 0.0f, float det_time = 0.0f)
    ...
    static StatusResponse from_message(const Message& msg) {
        ...
        // Check if we have enough data for new fields (backward compatibility)
        if (offset < msg.payload.size()) {
            max_ratio = msg.read_float(offset);
            max_deg = msg.read_float(offset);
            min_size = msg.read_uint32(offset);
            d_th = msg.read_float(offset);
            f_th = msg.read_float(offset);
        }
        
        if (offset < msg.payload.size()) {
            det_time = msg.read_float(offset);
        }

        return StatusResponse(cam, rec, train, people, faces, current_fps,
                            max_ratio, max_deg, min_size, d_th, f_th, det_time);
    }
};
...
struct PersonInfo {
    ...
    void serialize(Message& msg) const {
        ...
        // Write timestamp as uint64_t directly now that we have support
        msg.write_uint64(created_timestamp);
    }

    static PersonInfo deserialize(const Message& msg, size_t& offset) {
        PersonInfo info;
        info.name = msg.read_string(offset);
        info.id = msg.read_uint64(offset);
        info.image_count = msg.read_uint32(offset);
        info.created_timestamp = msg.read_uint64(offset);
        return info;
    }
};
