...
void update_title_bar_location(int screen_id) {
    ...
    for (int i = 0; i <= screen_stack_top && i < MAX_SCREENS; i++) {
        ...
        switch (screen_stack[i].screen_id) {
            ...
            case SCREEN_CAMERA:
                {
                    char key[64];
                    snprintf(key, sizeof(key), "menu_items.%s", "camera");
                    name = get_label(key);
                }
                break;
    ...
    lv_obj_t *label = app_state_get_current_title_label() ? app_state_get_current_title_label() : app_state_get_title_label();
...
void show_screen(int screen_id) {
    ...
    if (screen_id == SCREEN_MAIN) {
        ...
        if (screen_stack_top >= 0 && screen_stack[screen_stack_top].screen_id != SCREEN_MAIN) {
            inactivity_timer_set_last_screen(screen_stack[screen_stack_top].screen_id);
        }
        ...
        inactivity_timer_set_last_screen(screen_id);
...
lv_obj_t *create_standard_title_bar(lv_obj_t *parent, int screen_id) {
    ...
    app_state_set_current_title_label(title_label);
...
lv_obj_t *create_standard_status_bar(lv_obj_t *parent) {
    ...
    if (!app_state_get_status_bar()) {
        app_state_set_status_bar(lv_obj_create(parent));
        lv_obj_set_size(app_state_get_status_bar(), SCREEN_WIDTH, STATUS_BAR_HEIGHT);
        lv_obj_align(app_state_get_status_bar(), LV_ALIGN_BOTTOM_MID, 0, 0);
        apply_bar_style(app_state_get_status_bar(), get_status_bar_color());
        
        // Set user data to identify as status bar
        lv_obj_set_user_data(app_state_get_status_bar(), (void*)2);  // ID: 2 = status bar

        // Ensure status bar is always visible and on top
        lv_obj_clear_flag(app_state_get_status_bar(), LV_OBJ_FLAG_HIDDEN);
        lv_obj_move_foreground(app_state_get_status_bar());

        // Initialize status icons array
        for (int i = 0; i < MAX_STATUS_ICONS; i++) {
            app_state_set_status_icon(i, NULL);
        }
    } else {
        // Move the existing status bar to the new parent screen
        lv_obj_set_parent(app_state_get_status_bar(), parent);
        lv_obj_align(app_state_get_status_bar(), LV_ALIGN_BOTTOM_MID, 0, 0);
    ...
    return app_state_get_status_bar();
}
...
void finalize_screen(lv_obj_t *screen, int screen_id) {
    ...
    lv_obj_add_event_cb(screen, inactivity_non_home_activity_cb, LV_EVENT_PRESSED, NULL);
    lv_obj_add_event_cb(screen, inactivity_non_home_activity_cb, LV_EVENT_CLICKED, NULL);
 ...
 * Loads enabled icons from configuration and positions them in fixed slots (0-4).
 * First added button goes to slot 0, second to slot 1, etc.
 */
void update_status_bar_icons(void) {
    ...
    lv_obj_clean(app_state_get_status_bar());
    ...
    for (int i = 0; i < MENU_ITEMS_COUNT; i++) {
        if (app_state_is_menu_item_selected(i)) {
            ...
            if (slot_position < 0 || slot_position >= MAX_STATUS_ICONS) {
                continue;  // Skip invalid orders
            }

            // Calculate X position for this slot
            // Slot position = EDGE_MARGIN + (slot_position * SLOT_WIDTH) + ICON_OFFSET_IN_SLOT
            int slot_x = STATUS_BAR_EDGE_MARGIN + (slot_position * STATUS_BAR_SLOT_WIDTH) + STATUS_BAR_ICON_OFFSET;
            
            // Create button for this icon
            lv_obj_t *icon_btn = lv_btn_create(app_state_get_status_bar());
            lv_obj_set_size(icon_btn, STATUS_BAR_ICON_SIZE, STATUS_BAR_ICON_SIZE);
            lv_obj_set_pos(icon_btn, slot_x, (STATUS_BAR_HEIGHT - STATUS_BAR_ICON_SIZE) / 2);
    ...
    lv_obj_invalidate(app_state_get_status_bar());
}
...
void move_status_bar_to_screen(lv_obj_t *screen, int screen_id) {
    ...
    if (app_state_get_status_bar() && screen) {
        lv_obj_set_parent(app_state_get_status_bar(), screen);
        lv_obj_align(app_state_get_status_bar(), LV_ALIGN_BOTTOM_MID, 0, 0);
        lv_obj_clear_flag(app_state_get_status_bar(), LV_OBJ_FLAG_HIDDEN);
        lv_obj_move_foreground(app_state_get_status_bar());
    }
