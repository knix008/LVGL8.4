    ...
    if (status_bar && *status_bar == '{') {
        ...
        for (int i = 0; i < MENU_ITEMS_COUNT; i++) {
            ...
            if (value) {
                ...
                if (parse_bool(value)) {
                    app_state_get_internal()->menu_item_order[i] = current_order++;
                } else {
                    app_state_get_internal()->menu_item_order[i] = -1;
                }
            } else {
...
uint32_t get_background_color(void) {
    return app_state_get_internal()->bg_color ? app_state_get_internal()->bg_color : COLOR_BG_DARK;
}
...
uint32_t get_title_bar_color(void) {
    return app_state_get_internal()->title_bar_color ? app_state_get_internal()->title_bar_color : COLOR_BG_TITLE;
}
...
uint32_t get_status_bar_color(void) {
    return app_state_get_internal()->status_bar_color ? app_state_get_internal()->status_bar_color : COLOR_BG_TITLE;
}
...
uint32_t get_button_color(void) {
    return app_state_get_internal()->button_color ? app_state_get_internal()->button_color : COLOR_BUTTON_BG;
}
...
uint32_t get_button_border_color(void) {
    return app_state_get_internal()->button_border_color ? app_state_get_internal()->button_border_color : COLOR_BORDER;
}
...
int save_theme_config(void) {
    ...
    fprintf(file, "    \"selected_date\": \"%04d-%02d-%02d\"\n", app_state_get_internal()->calendar_date.year, app_state_get_internal()->calendar_date.month, app_state_get_internal()->calendar_date.day);
    fprintf(file, "  },\n");

    // Write fonts section
    fprintf(file, "  \"fonts\": {\n");
    fprintf(file, "    \"title\": {\n");
    fprintf(file, "      \"name\": \"%s\",\n", app_state_get_internal()->font_name_title);
    fprintf(file, "      \"size\": %d\n", app_state_get_internal()->font_size_title_bar);
    fprintf(file, "    },\n");
    fprintf(file, "    \"status_bar\": {\n");
    fprintf(file, "      \"name\": \"%s\",\n", app_state_get_internal()->font_name_status_bar);
    fprintf(file, "      \"size\": %d\n", app_state_get_internal()->font_size_status_bar);
    fprintf(file, "    },\n");
    fprintf(file, "    \"button_label\": {\n");
    fprintf(file, "      \"name\": \"%s\",\n", app_state_get_internal()->font_name_button_label);
    fprintf(file, "      \"size\": %d\n", app_state_get_internal()->font_size_button_label);
    fprintf(file, "    },\n");
    fprintf(file, "    \"label\": {\n");
    fprintf(file, "      \"name\": \"%s\",\n", app_state_get_internal()->font_name_label);
    fprintf(file, "      \"size\": %d\n", app_state_get_internal()->font_size_label);
    fprintf(file, "    },\n");
    fprintf(file, "    \"home_contents\": {\n");
    fprintf(file, "      \"name\": \"%s\",\n", app_state_get_internal()->font_name_home_contents);
    fprintf(file, "      \"size\": %d\n", app_state_get_internal()->font_size_home_contents);
    fprintf(file, "    }\n");
    fprintf(file, "  }\n");
    fprintf(file, "}\n");
...
int load_theme_config(void) {
    ...
    if (!content) {
        app_state_get_internal()->bg_color = COLOR_BG_DARK;
        ...
        app_state_get_internal()->calendar_date.year = tm->tm_year + 1900;
        app_state_get_internal()->calendar_date.month = tm->tm_mon + 1;
        app_state_get_internal()->calendar_date.day = tm->tm_mday;
        return 0;
    }
    ...
    if (theme && *theme == '{') {
        ...
        if (bg_color) {
            ...
            app_state_get_internal()->bg_color = strtoul(bg_color, NULL, 0);
        } else {
            app_state_get_internal()->bg_color = COLOR_BG_DARK;
        }
        ...
        if (title_color) {
            ...
            app_state_get_internal()->title_bar_color = strtoul(title_color, NULL, 0);
        } else {
            app_state_get_internal()->title_bar_color = COLOR_BG_TITLE;
        }
        ...
        if (status_color) {
            ...
            app_state_get_internal()->status_bar_color = strtoul(status_color, NULL, 0);
        } else {
            app_state_get_internal()->status_bar_color = COLOR_BG_TITLE;
        }
        ...
        if (button_color) {
            ...
            app_state_get_internal()->button_color = strtoul(button_color, NULL, 0);
        } else {
            app_state_get_internal()->button_color = COLOR_BUTTON_BG;
        }
        ...
        if (button_border) {
            ...
            app_state_get_internal()->button_border_color = strtoul(button_border, NULL, 0);
        } else {
            app_state_get_internal()->button_border_color = COLOR_BORDER;
        }
        ...
        if (language) {
            ...
            if (strcmp(lang_buf, "ko") == 0 || strcmp(lang_buf, "en") == 0) {
                snprintf(app_state_get_internal()->current_language, sizeof(app_state_get_internal()->current_language), "%s", lang_buf);
            } else {
                snprintf(app_state_get_internal()->current_language, sizeof(app_state_get_internal()->current_language), "ko");
            }
        } else {
    ...
    if (calendar_section && *calendar_section == '{') {
        ...
        if (selected_date) {
            ...
            if (sscanf(selected_date, "%d-%d-%d", &year, &month, &day) == 3) {
                app_state_get_internal()->calendar_date.year = year;
                app_state_get_internal()->calendar_date.month = month;
                app_state_get_internal()->calendar_date.day = day;
            } else {
...
int load_font_config(void) {
    ...
    if (!content){
        app_state_get_internal()->font_size_title_bar = FONT_SIZE_TITLE_BAR;
        app_state_get_internal()->font_size_status_bar = FONT_SIZE_REGULAR;
        app_state_get_internal()->font_size_button_label = FONT_SIZE_BUTTON;
        app_state_get_internal()->font_size_label = FONT_SIZE_REGULAR;
        app_state_get_internal()->font_size_home_contents = 30;
        app_state_get_internal()->font_size_bold = FONT_SIZE_BOLD;
        strncpy(app_state_get_internal()->font_name_title, "NotoSansKR-Bold.ttf", 63);
        strncpy(app_state_get_internal()->font_name_status_bar, "NotoSansKR-Regular.ttf", 63);
        strncpy(app_state_get_internal()->font_name_button_label, "NotoSansKR-Medium.ttf", 63);
        strncpy(app_state_get_internal()->font_name_label, "NotoSansKR-Regular.ttf", 63);
        strncpy(app_state_get_internal()->font_name_home_contents, "NotoSansKR-Bold.ttf", 63);
        app_state_get_internal()->font_name_title[63] = '\0';
        app_state_get_internal()->font_name_status_bar[63] = '\0';
        app_state_get_internal()->font_name_button_label[63] = '\0';
        app_state_get_internal()->font_name_label[63] = '\0';
        app_state_get_internal()->font_name_home_contents[63] = '\0';
        return 0;
    }
    ...
    if (fonts && *fonts == '{') {
        ...
        if (title && *title == '{') {
            ...
            if (title_name) {
                ...
                while (*title_name && *title_name != '"' && i < 63) {
                    app_state_get_internal()->font_name_title[i++] = *title_name++;
                }
                app_state_get_internal()->font_name_title[i] = '\0';
            } else {
                strncpy(app_state_get_internal()->font_name_title, "NotoSansKR-Bold.ttf", 63);
                app_state_get_internal()->font_name_title[63] = '\0';
            }
            ...
            if (title_size) {
                ...
                app_state_get_internal()->font_size_title_bar = atoi(title_size);
            } else {
                app_state_get_internal()->font_size_title_bar = FONT_SIZE_TITLE_BAR;
            }
        } else {
        ...
        if (status_bar && *status_bar == '{') {
            ...
            if (status_name) {
                ...
                while (*status_name && *status_name != '\"' && i < 63) {
                    app_state_get_internal()->font_name_status_bar[i++] = *status_name++;
                }
                app_state_get_internal()->font_name_status_bar[i] = '\0';
            } else {
                strncpy(app_state_get_internal()->font_name_status_bar, "NotoSansKR-Regular.ttf", 63);
                app_state_get_internal()->font_name_status_bar[63] = '\0';
            }
            ...
            if (status_size) {
                ...
                app_state_get_internal()->font_size_status_bar = atoi(status_size);
            } else {
                app_state_get_internal()->font_size_status_bar = FONT_SIZE_REGULAR;
            }
        } else {
        ...
        // Parse label font
        const char* label = find_json_value(fonts, "label");
        if (label && *label == '{') {
            const char* label_name = find_json_value(label, "name");
            if (label_name) {
                while (*label_name && (*label_name == '\"' || isspace(*label_name))) label_name++;
                int i = 0;
                while (*label_name && *label_name != '\"' && i < 63) {
                    app_state_get_internal()->font_name_label[i++] = *label_name++;
                }
                app_state_get_internal()->font_name_label[i] = '\0';
            } else {
                strncpy(app_state_get_internal()->font_name_label, "NotoSansKR-Regular.ttf", 63);
                app_state_get_internal()->font_name_label[63] = '\0';
            }

            const char* label_size = find_json_value(label, "size");
            if (label_size) {
                while (*label_size && !isdigit(*label_size)) label_size++;
                app_state_get_internal()->font_size_label = atoi(label_size);
            } else {
                app_state_get_internal()->font_size_label = FONT_SIZE_REGULAR;
            }
        } else {
        ...
        if (button_label && *button_label == '{') {
            ...
            if (button_name) {
                ...
                while (*button_name && *button_name != '\"' && i < 63) {
                    app_state_get_internal()->font_name_button_label[i++] = *button_name++;
                }
                app_state_get_internal()->font_name_button_label[i] = '\0';
            } else {
                strncpy(app_state_get_internal()->font_name_button_label, "NotoSansKR-Medium.ttf", 63);
                app_state_get_internal()->font_name_button_label[63] = '\0';
            }
            ...
            if (button_size) {
                ...
                app_state_get_internal()->font_size_button_label = atoi(button_size);
            } else {
                app_state_get_internal()->font_size_button_label = FONT_SIZE_BUTTON;
            }
        } else {
        ...
        // Parse home_contents font
        const char* home_contents = find_json_value(fonts, "home_contents");
        if (home_contents && *home_contents == '{') {
            const char* home_contents_name = find_json_value(home_contents, "name");
            if (home_contents_name) {
                while (*home_contents_name && (*home_contents_name == '\"' || isspace(*home_contents_name))) home_contents_name++;
                int i = 0;
                while (*home_contents_name && *home_contents_name != '\"' && i < 63) {
                    app_state_get_internal()->font_name_home_contents[i++] = *home_contents_name++;
                }
                app_state_get_internal()->font_name_home_contents[i] = '\0';
            } else {
                strncpy(app_state_get_internal()->font_name_home_contents, "NotoSansKR-Bold.ttf", 63);
                app_state_get_internal()->font_name_home_contents[63] = '\0';
            }

            const char* home_contents_size = find_json_value(home_contents, "size");
            if (home_contents_size) {
                while (*home_contents_size && !isdigit(*home_contents_size)) home_contents_size++;
                app_state_get_internal()->font_size_home_contents = atoi(home_contents_size);
            } else {
                app_state_get_internal()->font_size_home_contents = 30;
            }
        } else {
