 ...
 * Sets font to label font and standard text color.
 * 
...
void apply_label_style(lv_obj_t *label) {
    ...
    if (app_state_get_font_label()) {
        lv_obj_set_style_text_font(label, app_state_get_font_label(), 0);
    } else if (app_state_get_font_20()) {
        // Fallback to title font if label font not available
        lv_obj_set_style_text_font(label, app_state_get_font_20(), 0);
    }
}

/**
 * Applies status bar label styling with status bar font.
 * 
 * @param label The label object to style
 */
void apply_status_bar_label_style(lv_obj_t *label) {
    uint32_t label_color = app_state_get_label_text_color();
    if (label_color == 0) {
        label_color = COLOR_TEXT;  // Fallback to default if not set
    }
    lv_obj_set_style_text_color(label, lv_color_hex(label_color), 0);
    if (app_state_get_font_status_bar()) {
        lv_obj_set_style_text_font(label, app_state_get_font_status_bar(), 0);
    } else if (app_state_get_font_button()) {
        // Fallback to button font if status bar font not available
