...
static lv_obj_t *create_admin_content(lv_obj_t *parent) {
    ...
    lv_label_set_text(title_label, "테마 색상 설정");
    ...
    create_color_section(content, "배경 색상", 50, COLOR_TARGET_BACKGROUND);
    create_color_section(content, "제목 막대 색상", 130, COLOR_TARGET_TITLE_BAR);
    create_color_section(content, "상태 막대 색상", 210, COLOR_TARGET_STATUS_BAR);
    create_color_section(content, "버튼 색상", 290, COLOR_TARGET_BUTTON);
    create_color_section(content, "버튼 테두리 색상", 370, COLOR_TARGET_BUTTON_BORDER);

    // Info text at bottom
    lv_obj_t *info_label = lv_label_create(content);
    lv_label_set_long_mode(info_label, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(info_label, SCREEN_WIDTH - CONTENT_WIDTH_PADDING);
    lv_label_set_text(info_label, "\n선택한 색상이 자동 저장되며 즉시 적용됩니다.\n");
    ...
    lv_obj_align(info_label, LV_ALIGN_BOTTOM_LEFT, 
