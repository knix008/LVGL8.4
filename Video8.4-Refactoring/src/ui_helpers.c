...
static lv_obj_t* create_button_internal(lv_obj_t* parent, const char* text,
    ...
    // Apply font if requested and available
    if (apply_font) {
...
lv_obj_t* create_button_with_label(lv_obj_t* parent, const char* text,
                                   int width, int height, uint32_t bg_color,
                                   lv_event_cb_t callback, void* user_data) {
    return create_button_internal(parent, text, width, height, bg_color,
